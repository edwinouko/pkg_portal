...
// Get DataBase Values for Chart
					...
					function queryDBFilter($connection,$queryS, $queryE, $pArray, $gArray, $rArray, $dArray, $aArray, $deArray) {
						$sql = $queryS."WHERE PKGProgram IN ('".$pArray."')"." AND Gender IN ('".$gArray."')"." AND Race IN ('".$rArray."')".
						" AND StudentType IN ('".$dArray."')"." AND ProgramYear IN ('".$aArray."')"." AND Department IN ('".$deArray."')".$queryE;
	...
	$reqType = $_POST['reqType'];
		...
		if ($reqType=="allData"){
					...
					if (mysqli_connect_errno()){
						...
						//$stmt = $con->prepare("SELECT GENDER, COUNT(GENDER) AS tot FROM Registration GROUP BY GENDER");
						//$stmt->bind_param('sss', $filter,$filter,$filter);
						
						//$query = "SELECT GENDER, COUNT(GENDER) AS tot FROM Registration GROUP BY GENDER";
			   ...
			   <h2>Hmm, something went wrong. Please try again.</h2></div>";
				...
				echo json_encode(array(
					...
					if (mysqli_connect_errno()){
						...
						// Contribute
						$query = "SELECT Q1Contribute_SI FROM CompletionData";
						$Q1_SI= queryDB($con, $query);

						$query = "SELECT Q1Contribute_USC FROM CompletionData";
						$Q1_USC= queryDB($con, $query);
									
						$query = "SELECT Q1Contribute_Skill FROM CompletionData";
						$Q1_Skill= queryDB($con, $query);

						$query = "SELECT Q1Contribute_Network FROM CompletionData";
						$Q1_Network= queryDB($con, $query);

						$query = "SELECT Q1Contribute_Res FROM CompletionData";
						$Q1_Res= queryDB($con, $query);
				
						// Interest
						$query = "SELECT Q2Interest_Context FROM CompletionData";
						$Q2_Context= queryDB($con, $query);
						
						$query = "SELECT Q2Interest_USC FROM CompletionData";
						$Q2_USC= queryDB($con, $query);

						$query = "SELECT Q2Interest_Skill FROM CompletionData";
						$Q2_Skill= queryDB($con, $query);

						$query = "SELECT Q2Interest_Res FROM CompletionData";
						$Q2_Res= queryDB($con, $query);
	
						$query = "SELECT Q2Interest_Network FROM CompletionData";
						$Q2_Network= queryDB($con, $query);
				
						// Equip 
						$query = "SELECT Q3Equip_Vol FROM CompletionData";
						$Q3_Vol= queryDB($con, $query);
				
						$query = "SELECT Q3Equip_Community FROM CompletionData";
						$Q3_Com= queryDB($con, $query);

						$query = "SELECT Q3Equip_Career FROM CompletionData";
